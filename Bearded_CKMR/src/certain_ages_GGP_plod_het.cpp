...
template<class Type>
Type get_PPO_prob(int age_p,int cy,int dy,vector<Type> M_a, vector<Type> N_a_yr,Type male_scale){
  ...
  return ideath*2.0*M_a(age_p)/(repro_tot*male_scale);
}
...
template<class Type>
Type get_PHS_prob(int n_ages,int delta_yr,vector<Type>N_a_yri,vector<Type>N_a_yrj,vector<Type>S_a,vector<Type>m_a,Type male_scale){
  ...
  return 2.0*cum_prob/(rel_repro1*rel_repro2*male_scale);  //2.0 is because of 50/50 age structure
}
...
Type get_f_mt_GGP_prob(int n_ages,int bi, int bj, int di, int min_repro_fem,matrix<Type>N_a,vector<Type>S_a,vector<Type>f_a){
  ...
  Type tot_prob = 0.0;
  if(delta_yr>=min_repro_gap && di>=(bi+min_repro_fem)){  //GGP can only happen if there's time for two generations and of Grandmother lives to time of mother's first possible birth
    Type phi = 1.0;
    Type cum_prob = 0.0;
...
Type get_f_nomt_GGP_prob(int n_ages,int bi, int bj, int di, int min_repro_fem,int min_repro_male,matrix<Type>N_a,vector<Type>S_a,vector<Type>f_a,vector<Type>m_a){
  ...
  Type tot_prob = 0.0;
  if(delta_yr>=min_repro_gap && di>=(bi+min_repro_male-1)){  //GGP can only happen if there's time for two generations and of Grandmother lives to time of father's first possible birth
    Type phi = 1.0;
    Type cum_prob = 0.0;
    Type tot_repro_k = 0.0;
    Type tot_repro_j = 0.0;
...
Type get_m_nomt_GGP_prob(int n_ages,int bi, int bj, int di, int min_repro_fem,int min_repro_male,matrix<Type>N_a,vector<Type>S_a,vector<Type>f_a,vector<Type>m_a){
  ...
  Type tot_prob = 0.0;
  if(delta_yr>=min_repro_gap && di>=(bi+min_repro_male-1)){  //GGP can only happen if there's time for two generations 
    Type phi = 1.0;
    Type cum_prob = 0.0;
    Type tot_repro_k = 0.0;
    Type tot_repro_j = 0.0;
  ...
  Type tot_prob2 = 0.0;
  if(delta_yr>=min_repro_gap && di>=(bi+min_repro_fem)){  //GGP can only happen if there's time for two generations 
    Type phi = 1.0;
    Type cum_prob = 0.0;
    Type tot_repro_k = 0.0;
    Type tot_repro_j = 0.0;
...
Type objective_function<Type>::operator() ()
{
  ...
  PARAMETER(log_eta2);
  PARAMETER(log_eta3);
  PARAMETER(male_scale_logit);  //heterogeneity parameter used to reduce 
  
  vector<Type> N(n_yrs);
  Type eta1 = exp(log_eta1);
  Type eta2 = exp(log_eta2);
  Type eta3 = exp(log_eta3);
  Type male_scale = plogis(male_scale_logit);
  ...
  for(int ibi=1;ibi<(n_yrs-1);ibi++){ //need access to previous year abundance for male probs
    for(int ibj=ibi+1; ibj<std::min(n_yrs,ibi+n_ages); ibj++){
      ...
      PHS_table(ibi,ibj)=get_PHS_prob(n_ages,delta_yr,N_a_yri_min1,N_a_yrj_min1,S_a,Male_mat,male_scale)*p_plod;
      MHS_table(ibi,ibj)=get_MHS_prob(n_ages,delta_yr,N_a_yri,N_a_yrj,S_a,Fem_fec)*p_plod;
      for(int idi=0;idi<n_yrs_data;idi++){
        int dy = idi+n_ages;
        GGP_table(0,ibi,idi,ibj,1)=get_f_mt_GGP_prob(n_ages,ibi,ibj,dy,min_repro_fem,N_a,S_a,Fem_fec)*p_plod;  //note male_scale cancels out of GGP
        ...
        PPO_table(ibi,idi,ibj)=get_PPO_prob(delta_yr-1,ibi-1,dy,Male_mat,N_a_yri_min1,male_scale); //a year earlier since breeding occurs ~11 months before pups born
      }
    }
    int delta_yr = 0;
    vector<Type>N_a_yri_min1=N_a.row(ibi-1);
    PHS_table(ibi,ibi)=get_PHS_prob(n_ages,delta_yr,N_a_yri_min1,N_a_yri_min1,S_a,Male_mat,male_scale)*p_plod;
  }
