...
goal is to fit a number of different types of CKMR models, which progress in their complexity. First,
...
```{r tmb,tidy='styler',cache=T,include=F}
  ...
  library(TMB)
  getwd()
  TmbFile = "../src/certain_ages_1gen.cpp"
  compile(TmbFile )
  TmbExec="../src/certain_ages_1gen"
...
Pr(GGP,m_{ij}=1|s_i=0,d_i,b_i,b_j) = \sum_{t=b_i}^{\min(d_i,b_j)} \frac{f_{t-b_i} N_{t,0}}{\sum_a  f_a N_{t,a}}
  \frac{ \{ \prod_{k=t}^{b_i-1} \phi_{k-t} \} 2 f_{b_j-t}}{\sum_a f_a N_{b_j,a}} = \sum_{t=b_i}^{\min(d_i,b_j)} \frac{f_{t-b_i} N_{b_j,b_j-t}}{\sum_a  f_a N_{t,a}}
...
$$
Pr(GGP, m_{ij}=0|s_i=0,d_i,b_i,b_j) = \sum_{t=b_i}^{\min(d_i,b_j-1)} \frac{f_{t-b_i}}{\sum_a  f_a N_{t,a}}
  \frac{ N_{b_j-1,b_j-t-1} 2m_{b_j-t-1}}{\sum_a m_a N_{b_j-1,a}}
$$
...
$$
\begin{aligned}
Pr(GGP, m_{ij}=0|s_i=1,d_i,b_i,b_j) &=  \sum_{t=b_i+1}^{\min(d_i,b_j)} \frac{m_{t-b_i-1} }{\sum_a  m_a N_{t-1,a}}
  \frac{ N_{b_j,b_j-t} 2f_{b_j-t}}{\sum_a f_a N_{b_j,a}} \\
  &+ 
...
library(TMB)
TmbFile_GGP = "../src/certain_ages_GGP.cpp"
compile(TmbFile_GGP )
TmbExec_GGP="../src/certain_ages_GGP"
...
library(TMB)
TmbFile_plod = "../src/certain_ages_GGP_plod.cpp"
compile(TmbFile_plod )
TmbExec_plod="../src/certain_ages_GGP_plod"
...
N_est_TMB_40 = SD_report_40$value[which(names(SD_report_40$value)=="N")]
SD_N_40 = SD_report_40$sd[which(names(SD_report_40$value)=="N")]
...
Looking at our data, thre are a total of 17 HSP/GGPs that don't share mtDNA, and 8 that do share mtDNA.  If we look at these records, it looks like there are 4 that have a chance of being GGPs, but in each case they are more likely to be HSPs.  If we assume that they are all HSPs, and also assume that we are equally likely to detect maternal and paternal HSPs, the chance of observing so few maternal HSPs is `r pbinom(8,25,0.5)`.  This could have happened by random chance, but it also may have happened because of heterogeneity in male reproductive success (e.g., if older or higher quality males are able to breed with more females than younger or lower quality males).  Bearded seals are known
to maintain underwater territories during breeding season, and it may be the case that there may be some competition for mates.  

In order to account for this possibility, we conducted a sensitivity scenario where we assumed the number of male 
breeders is an unknown fraction ($\pi$) of the total number of reproductively mature males.  Specifically, we set $N_{t,a}^M = 0.5 N_{t,a} \pi$ every where that male abundance appears in previous calculations. 


```{r,male_het,tidy='styler',cache=T,include=FALSE}
Data=Data40
Data$lambda_expect = 1.0
Params$male_scale_logit = 2.0 

TmbFile_het = "../src/certain_ages_GGP_plod_het.cpp"
compile(TmbFile_het )
TmbExec_het="../src/certain_ages_GGP_plod_het"
...
reproductively mature males are successfully producing offspring each year. As expected, this leads to an increase in estimated abundance, which is now at $\hat{N}=$ `r N_est_TMB_het[1]`.


Let's take a look at a summary of estimated abundance from this combination of sensitivity runs.  For the increasing and decreasing population
...
N_df$CV = c(SD_N_40[1]/Report_40$N[1],SD_N_het[1]/Report_het$N[1],SD_N_50[1]/Report_50$N[1],SD_N_30[1]/Report_30$N[1],
